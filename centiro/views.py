...
def index(request):
    ...
    template = loader.get_template('centiro/index.html')
    context = {
...
def detail(request, site_name):
    ...
    return render(request, 'centiro/detail.html', {'site' : site})

def contacts(request, site_name):
    site = get_object_or_404(Sites, site_name=site_name)
    return render(request, 'centiro/contacts.html', {'site' : site})

def environment(request, site_name):
    site = get_object_or_404(Sites, site_name=site_name)
    return render(request, 'centiro/environment.html', {'site' : site})

def dependent_service(request, site_name):
    site = get_object_or_404(Sites, site_name=site_name)
    return render(request, 'centiro/dependent_service.html', {'site' : site})


def environment_detail(request,site_name, environment_description):
    #err
    template = loader.get_template('centiro/environment_detail.html')
